    ...
    def register_button_on_click():
        if password1.value == password2.value:
            if donor_or_volunteer.value == "Donor":
                dbm.addDonor(username.value, password1.value, name.value, surname.value, email.value)
            elif donor_or_volunteer.value == "Volunteer":
                dbm.Volunteer(username.value, password1.value, name.value, surname.value, email.value)
            ui.navigate.to("/login")
        else:
...
def donor_menu():
    ...
    if(current_donor is None):
        ...
        ui.button(text="Add Donation", on_click=lambda: ui.navigate.to("/add-donation"))
        shipment_request_table = ui.table(columns=shipment_request_columns,rows=[])
        ui.button(text="Add Shipment Request", on_click=lambda: ui.navigate.to("/add-shipment-request"))
...
def add_donation():
    ...
    if current_donor is None:
        ...
        def add_donation_button_on_click():
            dbm.addDonation(type_of_donation.value, float(amount.value), area.value, current_donor)
            ui.navigate.to("/donor-menu")


        ui.button(text="Add Donation",on_click=lambda: add_donation_button_on_click())
    else:
        ui.notify("Identification failed.")
...
def add_shipment_request():
    ...
    if current_donor is None:
        ...
        selfShipped_or_shippedByNGO=ui.toggle(["Self Shipped","Shipped by NGO"],value="Self Shipped")
        ...
        def add_shipment_request_button_on_click():
            ...
            if current_donation is None:
                ...
                if selfShipped_or_shippedByNGO=="Self Shipped":
                    dbm.addShipmentRequest(address.value, True, current_donation, current_donor)
                elif selfShipped_or_shippedByNGO=="Shipped by NGO":
                    dbm.addShipmentRequest(address.value, False, current_donation, current_donor)

 
        ui.button(text="Add Shipment Request", on_click=lambda: add_shipment_request_button_on_click())
    else:
        ui.notify("Identification failed.")
#Volunteer Part
...
def volunteer_menu():
    ...
    if current_volunteer is None:
        ...
        ui.button(text="Set Personal Profile", on_click=ui.navigate.to("/set-personal-profile"))
    else:
        ui.notify("Identification failed.")
